...
const createUploadStory = catchAsyncError(async (req, res, next) => {
    const mediaFiles = req.files;
    const videoFilesTypes = [".mp4", ".mkv"];

    if (!req.body.blur) return next(new ErrorHandler(ResponseMessages.BLUR_REQUIRED, 400));
  
    if (!req.body.semi_blur) return next(new ErrorHandler(ResponseMessages.SEMI_BLUR_REQUIRED, 400));  
    ...
    for (let i = 0; i < mediaFiles.length; i++) {
        ...
        if (videoFilesTypes.includes(fileExt)) {
            await cloudinary.v2.uploader
                .upload(fileTempPath, {
                    folder: "social_media_api/stories/videos",
                    resource_type: "video",
                    chunk_size: 6000000,
                })
                .then(async (result) => {
            ...
            await cloudinary.v2.uploader
                .upload(fileTempPath, {
                    folder: "social_media_api/stories/images",
                })
                .then(async (result) => {
                    mediaFilesLinks.push({
                        public_id: result.public_id,
    ...
    const newStory = {
        ...
        blur: req.body.blur,
        semi_blur: req.body.semi_blur,
        is_anonymous: req.body.is_anonymous !== undefined ? req.body.is_anonymous: false,
    };

    const story = await models.Story.create(newStory);
...
export default createUploadStory;
