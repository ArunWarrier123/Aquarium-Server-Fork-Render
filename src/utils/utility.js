...
// Check Vendor already Registered 
utility.checkVendorRegistered = async (phone) => {
...
utility.checkGroupNameAvailable = async (group_name) => {
  let group = await models.Group.findOne({ groupName: group_name });
...
utility.generateAuthToken = async (user, type) => {
  ...
  let authToken;
  if (type === "vendor") {
    authToken = await models.AuthToken.create({
      token: token,
      vendor: user._id,
...
utility.deleteExpiredStories = async () => {
  // console.log("[cron] task to delete expired Stories has started.");
  const stories = await models.Story.find({ expiredAt: { $lte: Date.now() }, storyStatus: 'active' }).select("_id storyStatus");

  if (stories.length > 0) {
    for (let i = 0; i < stories.length; i++) {
      stories[i].storyStatus = "deleted";
      await stories[i].save();
    }
    // console.log("[cron] Expired Stories deleted.");
  } else {
    // console.log("[cron] No Stories found.");
  }
};
...
utility.checkIfStoryOwner = async (storyId, user) => {
  const story = await models.Story.findById(storyId).select("owner");
...
utility.checkIfStoryLiked = async (storyId, user) => {
  const isLiked = await models.StoryLike.findOne({ post: storyId, user: user._id });
...
utility.getOwnerData = async (ownerId, reqUser) => {
  const owner = await models.User.findById(ownerId)
    .select([
      "_id", "fname", "lname", "email", "uname", "phone", "groups", "avatar", "profession",
  ...
  for (const groupId of owner.groups) {
...
utility.getProfileData = async (reqUser) => {
  const user = await models.User.findById(reqUser._id)
    .select([
      "_id", "fname", "lname", "email", "uname", "phone", "groups", "avatar",
...
utility.getUserProfileData = async (userId, reqUser) => {
  const user = await models.User.findById(userId)
    .select([
      "_id", "fname", "lname", "email", "uname", "phone", "groups", "avatar", "profession",
      "location", "postsCount", "followersCount", "followingCount", "about", "dob",
...
utility.getPostData = async (postId, reqUser) => {
  ...
  const group = await models.Group.findById(post.groupId);
  ...
  if (postType === "poll") {
    ...
    postData.is_anonymous = post.is_anonymous;
...
utility.getStoryData = async (storyId, reqUser) => {
  const story = await models.Story.findOne({ _id: storyId, storyStatus: "active" });

  if (!story) {
    return null;
  }

  const ownerData = await utility.getOwnerData(story.owner, reqUser);
  const isLiked = await utility.checkIfStoryLiked(story._id, reqUser);

  const group = await models.Group.findById(story.groupId);
  ...
  if (story.caption) {
    hashtags = utility.getHashTags(story.caption);
    mentions = utility.getMentions(story.caption);
  }

  storyData._id = story._id;

  storyData.caption = story.caption;
  storyData.mediaFiles = story.mediaFiles;

  storyData.group = group;
  storyData.owner = ownerData;

  storyData.hashtags = hashtags;
  storyData.userMentions = mentions;

  storyData.likesCount = story.likesCount;
  storyData.commentsCount = story.commentsCount;
  storyData.repostsCount = story.repostsCount;
  storyData.sharesCount = story.sharesCount;

  storyData.is_anonymous = story.is_anonymous;

  storyData.isLiked = isLiked;

  storyData.allowComments = story.allowComments;
  storyData.allowLikes = story.allowLikes;
  storyData.allowReposts = story.allowReposts;
  storyData.allowShare = story.allowShare;
  storyData.allowDownload = story.allowDownload;

  storyData.visibility = story.visibility;
  storyData.status = story.storyStatus;

  storyData.blur = story.blur;
  storyData.semi_blur = story.semi_blur;

  storyData.createdAt = story.createdAt;
  storyData.updatedAt = story.updatedAt;
  storyData.expiredAt = story.expiredAt;

  return storyData;
};


utility.getConferenceData = async (conferenceId, req_user) => {
  const conference = await models.Conference.findById(conferenceId);
  ...
  const speaker = await utility.getUserData(conference.speaker, req_user);

  const members = [];

  for (const memberId of conference.members) {
    const member = await utility.getUserData(memberId, req_user);
    members.push(member);
  }

  conferenceData._id = conference._id;

  conferenceData.title = conference.title;
  conferenceData.description = conference.description;

  conferenceData.isPrivate = conference.isPrivate;
  conferenceData.isLive = conference.isLive;

  conferenceData.speaker = speaker;
  conferenceData.members = members;

  conferenceData.broadcastedAt = conference.broadcastedAt;
  conferenceData.finishedAt = conference.finishedAt;

  conferenceData.createdAt = conference.createdAt;
  conferenceData.updatedAt = conference.updatedAt;

  return conferenceData;
};

utility.getVideoNotificationData = async (notificationId, req_user) => {
  ...
  const user = await utility.getUserData(notification.user._id, req_user);
  const senderUser = await utility.getUserData(notification.user._id, req_user);
  ...
  notificationData.isRequest = notification.isRequest;

  notificationData.senderUser = senderUser;

  notificationData.isRead = notification.isRead;
  notificationData.isConnected = notification.isConnected;
  notificationData.isDeclined = notification.isDeclined;
  notificationData.isLink = notification.isLink;
...
utility.getCommentData = async (commentId, reqUser) => {
  ...
  const post = await utility.getPostData(comment.post._id, reqUser);
  ...
  commentData.is_anonymous = comment.is_anonymous;
